...
| R'lyeh | Web Exploitation | <ul><li>Discovery of exposed environment configuration files (.env) via directory fuzzing to extract database credentials.</li><li>API endpoint reconnaissance and interaction using SwaggerUI documentation.</li><li>Identification of weak, deterministic token generation mechanisms (MD5 hashing of user signatures).</li><li>Privilege escalation to moderator (L3) level by forging tokens using leaked user attributes from unauthorized endpoints.</li></ul> | Burp Suite, CyberChef |
